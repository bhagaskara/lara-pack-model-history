...
class InjectHasCompleteHistoryCommand extends Command
{
    protected $signature = 'lara-pack:inject-has-complete-history
        {--path=app/Models : Path folder model} 
        {--remove : Hapus HasCompleteHistory dari model}';

    protected $description = 'Inject atau hapus HasCompleteHistory trait ke seluruh model';

    const TRAIT_NAMESPACE = 'LaraPack\ModelHistory\Traits\HasCompleteHistory';
    const TRAIT_NAME = 'HasCompleteHistory';
    ...
    protected function processFile($filePath, $isRemove = false)
    {
        ...
        if ($isRemove) {
            ...
            // 1) Hapus HasCompleteHistory dari "use ..." di dalam class (trait use)
            [$content, $changedClassUse] = $this->stripHasCompleteHistoryFromTraitUses($content, $classBodyStart);
            $modified = $modified || $changedClassUse;

            // 2) Hapus import di header (sebelum class)
            [$content, $changedImport] = $this->stripImportHasCompleteHistory($content, $classBodyStart);
            ...
            // 2) Tambahkan HasCompleteHistory ke trait use di dalam class
            if (preg_match('/class\s+\w+\s+extends\s+Model\b[^{]*\{/', $content, $m, PREG_OFFSET_CAPTURE)) {
                $classBodyStart = $m[0][1] + strlen($m[0][0]);
    ...
    /**
     * Menghapus HasCompleteHistory dari semua "use ...;" di dalam body class.
     * Menangani:
     *  - use HasFactory, HasCompleteHistory;
     *  - use HasFactory,
     *        HasCompleteHistory,
     *        SoftDeletes;
     *  - use HasCompleteHistory;
     *  - use HasCompleteHistory { ... }   (di-remove seluruh blok jika HasCompleteHistory satu-satunya trait)
     */
    private function stripHasCompleteHistoryFromTraitUses(string $content, int $classBodyStart): array
    {
        $changed = false;
        ...
        if (preg_match_all('/^\s*use\b[\s\S]*?;/m', $content, $matches, PREG_OFFSET_CAPTURE, $classBodyStart)) {
            ...
            foreach (array_reverse($matches[0]) as $m) {
                ...
                // Skip kalau tidak menyebut HasCompleteHistory
                ...
                // Filter keluarkan HasCompleteHistory (baik pendek atau FQCN)
     ...
     */
    private function stripImportHasCompleteHistory(string $content, int $classBodyStart): array
    {
        $changed = false;

        $header = substr($content, 0, $classBodyStart);
        if (strpos($header, 'use ' . self::TRAIT_NAMESPACE . ';') !== false) {
            $header = preg_replace('/\r?\n?use\s+App\\\\Traits\\\\HasCompleteHistory;/', '', $header, 1, $cnt);
            if ($cnt > 0) {
                $changed = true;
